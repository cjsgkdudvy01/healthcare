...
public class CustomAuthenticationSuccessHandler implements AuthenticationSuccessHandler{
	...
	private void clearAuthenticationAttributes(HttpServletRequest request) {
        HttpSession session = request.getSession(false);	//리퀘스트에 연관된 세션을 불러온다.

        if (session == null) {
            return;					//세션이 없으면 중단
        }

        session.removeAttribute(WebAttributes.AUTHENTICATION_EXCEPTION);	//세션에서 인증 예외 속성 즉 로그인 실패와같은 경우 저장된 에러를 지워준다.
    }
